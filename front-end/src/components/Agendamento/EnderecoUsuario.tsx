...
export default function EnderecoUsuario({ cep, bairro, rua, num, aoClicar, selecionado }: TipoEnderecoUsuario & { aoClicar: () => void, selecionado: boolean }) {
  return (
    <div onClick={aoClicar} className={`w-full h-full border-2 border-corPreto rounded-xl flex justify-center items-center py-10 px-4 cursor-pointer hover:bg-gray-100 transition duration-500 ${selecionado ? "scale-105 bg-cor2" : "hover:scale-105"}`}>
